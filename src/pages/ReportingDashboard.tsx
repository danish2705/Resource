...
export default function ResourceManagementOverview() {
  ...
  return (
    <div style={{ fontFamily: "system-ui,-apple-system,sans-serif", background: "#f3f4f6", minHeight: "100vh", padding: "16px 20px" }}>
      ...
      <div style={{ fontSize: 10, color: "#9ca3af", textAlign: "right", marginBottom: 12 }}>Last Updated: 15/05/26 10:30 AM</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 0.7fr 0.65fr 0.65fr", gap: 10, marginBottom: 14 }}>
        <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 10, padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            {[["Total Capacity",COLORS.blue],["Total Demand",COLORS.teal],["Gap (FTE)",COLORS.red]].map(([l,c]) => (<span key={l} style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 9, color: "#6b7280" }}><span style={{ width: 8, height: 8, borderRadius: 2, background: c }} />{l}</span>))}
          </div>
        </div>
          ...
          <div style={{ height: 160 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={utilizationTrend} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
                <defs><linearGradient id="utilGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={COLORS.purple} stopOpacity={0.4} /><stop offset="100%" stopColor={COLORS.purple} stopOpacity={0.05} /></linearGradient></defs>
                <CartesianGrid strokeDasharray="2 2" stroke="#f3f4f6" />
                <XAxis dataKey="month" tick={{ fontSize: 9 }} />
        ...
        <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 10, padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ position: "relative", width: 80, height: 80 }}>
              <PieChart width={80} height={80}><Pie data={workforceRisks} cx={39} cy={39} innerRadius={25} outerRadius={38} dataKey="value" startAngle={90} endAngle={-270}>{workforceRisks.map((r, i) => <Cell key={i} fill={r.color} />)}</Pie></PieChart>
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}><div style={{ fontSize: 14, fontWeight: 800, color: "#111" }}>24</div><div style={{ fontSize: 7, color: "#6b7280" }}>Total Risks</div></div>
            </div>
            <div style={{ flex: 1 }}>{workforceRisks.map((r, i) => (<div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 10, padding: "2px 0", color: "#374151" }}><span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 6, height: 6, borderRadius: 2, background: r.color }} />{r.name}</span><span style={{ fontWeight: 700, color: r.color }}>{r.value}</span></div>))}</div>
          </div>
          <div style={{ marginTop: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: "#374151", marginBottom: 4 }}>Top Risks</div>
            {riskTopItems.map((r, i) => (<div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "#6b7280", padding: "1px 0" }}><span>{r.icon} {r.text}</span><span style={{ fontWeight: 700, color: r.color }}>{r.count}</span></div>))}
          </div>
        </div>
        ...
        <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 10, padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 8 }}>
            <div style={{ position: "relative", width: 70, height: 70 }}>
              <PieChart width={70} height={70}><Pie data={[{value:78},{value:22}]} cx={34} cy={34} innerRadius={22} outerRadius={33} dataKey="value" startAngle={90} endAngle={-270}><Cell fill={COLORS.green} /><Cell fill="#e5e7eb" /></Pie></PieChart>
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 12, fontWeight: 800, color: COLORS.green }}>78%</span></div>
            </div>
            <div>{[["On Track","42%",COLORS.green],["At Risk","28%",COLORS.orange],["Off Track","18%",COLORS.red],["Not Started","12%",COLORS.gray]].map(([l,v,c]) => (<div key={l} style={{ display: "flex", gap: 6, fontSize: 9, alignItems: "center", color: "#6b7280", marginBottom: 2 }}><span style={{ width: 7, height: 7, borderRadius: 2, background: c }} />{l} <span style={{ color: c, fontWeight: 700 }}>{v}</span></div>))}</div>
          </div>
